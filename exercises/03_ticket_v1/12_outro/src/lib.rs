...
impl Order{
 ...
 fn validate_product_name(product_name: &String){
    if product_name.is_empty() {
        panic!("product name cant be empty");
    }

    if product_name.len() > 300 {
        panic!(" product name cant be more than 300 bytes")
    }
}

//here &u32 coz quantity is &u32 and we cant compare &u32 with just u32 (here zero)
